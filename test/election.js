...
contract("Election", function(accounts){
    ...
    it("it initializes the candidates with correct values", function() {
        ...
        .then(function(candidate) {
            ...
            assert.equal(candidate[1], "Bill", "contains correct name");
            assert.equal(candidate[2], 0, "contains correct votes count");
            return electionInstance.candidates(2);
        })
        .then(function(candidate) {
            assert.equal(candidate[0], 2, "contains correct id");
            assert.equal(candidate[1], "Hillary", "contains correct name");
            assert.equal(candidate[2], 0, "contains correct votes count");
            return electionInstance.candidates(3);
        })
        .then(function(candidate) {
            assert.equal(candidate[0], 3, "contains correct id");
            assert.equal(candidate[1], "John", "contains correct name");
            assert.equal(candidate[2], 0, "contains correct votes count");
            return electionInstance.candidates(4);
        })
        .then(function(candidate) {
            assert.equal(candidate[0], 4, "contains correct id");
            assert.equal(candidate[1], "Kacy", "contains correct name");
            assert.equal(candidate[2], 0, "contains correct votes count");
        })
    });
